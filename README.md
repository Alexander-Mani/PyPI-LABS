...
**`deployment.sh` aborts with `ERROR: benign_and_controlls.zip not found at …`.** The script looks for the sample zips in `${SAMPLES_DIR}` (defaults to `$HOME/samples` for the invoking operator). Either move your zips into that path or set `SAMPLES_DIR` to the directory that actually contains them — for example, `echo 'SAMPLES_DIR=/home/<your-user>/samples' >> .env` then re-run. Earlier versions of the script silently continued past an unzip failure, leaving an empty `samples/` tree and zero-count uploads at Step 9; the current script aborts loudly instead.
