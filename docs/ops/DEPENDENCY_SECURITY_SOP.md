...
Lockfiles secure the supply chain at a specific timestamp. They do not protect against retroactive vulnerability disclosures or delayed malware detection. The project uses `pip-audit` for continuous Software Composition Analysis. The execution pipeline runs this audit against the active lockfile before initiating any evaluation batch. The system aborts execution immediately if the audit returns known CVEs or malicious package flags.
